...
do
	...
	then
		...
		dx cd /
		dx run swiss-army-knife --ignore-reuse --name "$testName.toDo.$chr" -y --instance-type mem3_hdd2_v2_x4  -imount_inputs=FALSE -iin="/scripts/runGeneSetWithFileList.20230825.sh" -icmd="bash runGeneSetWithFileList.20230825.sh $testName $chr $testName.toDo.$chr.txt $otherNeededFileList"
	fi
done 
