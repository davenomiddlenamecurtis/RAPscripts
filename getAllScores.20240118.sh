...
# if something goes wrong and you need to change things and then start again,
# you may need to edit $resultsDir/$testName.scoresSoFar.txt and $failedDir/$testName.failedSoFar.txt
# NB the script does NOT check what results have previously been downloaded
...
do
	...
	then
		...
		dx cd /
		dx run swiss-army-knife --name "$testName.toDo.$chr" -y --instance-type mem3_hdd2_v2_x4  -imount_inputs=FALSE -iin="/scripts/runGeneSetWithFileList.20240118.sh" -icmd="bash runGeneSetWithFileList.20240118.sh $testName $chr $testName.toDo.$chr.txt $otherNeededFileList"
